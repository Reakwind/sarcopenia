# ============================================================================
# GENERATE SYNTHETIC TEST DATA (OPTIMIZED - INCREMENTAL WRITES)
# ============================================================================
#
# Generates realistic synthetic data for 50 patients × 10 visits
# Matches RedCap export schema from actual data
# OPTIMIZED: Writes rows incrementally instead of accumulating in memory
#
# ============================================================================
...
cat("Generating synthetic test data (optimized version)...\n")
...
cat("Creating output file:", output_file, "\n")

# Write header row
header_line <- paste0("\"", gsub("\"", "\"\"", column_names), "\"", collapse = ",")
...
for (patient_idx in 1:n_patients) {
  ...
  if (patient_idx %% 10 == 0) {
    cat("  Progress:", patient_idx, "/", n_patients, "patients (", total_rows, "rows written)\n")
  }
  ...
  for (visit_idx in 1:n_visits_per_patient) {
    ...
    # Create row vector
    row_data <- character(length(column_names))

    # Process each column
    for (col_idx in seq_along(column_names)) {
      ...
      row_data[col_idx] <- if (is.na(value)) "" else as.character(value)
    }

    # Write row to file (append mode)
    # Escape values with commas or quotes
    escaped_values <- sapply(row_data, function(val) {
      if (grepl(",|\"|\n", val)) {
        paste0("\"", gsub("\"", "\"\"", val), "\"")
      } else {
        val
    ...
    row_line <- paste(escaped_values, collapse = ",")
    write(row_line, file = output_file, append = TRUE)
...
cat("  Rows:", total_rows, "\n")
cat("  Columns:", length(column_names), "\n")
